...
public class FirstPersonController : MonoBehaviour,IGameObserver
{
    ...
    private void Update()
    {
        ...
        if(cameraCanMove)
        {
            ...
            if (!invertCamera)
            {
                pitch -= mouseSensitivity * Input.GetAxis(GameManager.instance.cameraVertical);
            }
            else
            {
                // Inverted Y
        ...
        if (enableZoom)
        {
            ...
            if(Input.GetButtonDown(GameManager.instance.zoomButton) && !holdToZoom && !isSprinting)
            {
                if (!isZoomed)
                {
                    isZoomed = true;
            ...
            {
                if(Input.GetButtonDown(GameManager.instance.zoomButton))
                {
                    isZoomed = true;
                }
                else if(Input.GetButtonUp(GameManager.instance.zoomButton))
                {
                    isZoomed = false;
                }
            }
        ...
        if(enableJump && Input.GetButtonDown(GameManager.instance.jumpButton) && isGrounded)
        {
            Jump();
        }
        ...
        if (enableCrouch)
        {
            ...
            else if(Input.GetButtonDown(GameManager.instance.crouchButton) && holdToCrouch && CanUnCrouch())
            {
                isCrouched = true;
                Crouch();
            }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            Vector3 targetVelocity = new Vector3(Input.GetAxis(GameManager.instance.horizontalAxis), 0, Input.GetAxis(GameManager.instance.verticalAxis));
            ...
            if (enableSprint && Input.GetButton(GameManager.instance.sprintButton) && sprintRemaining > 0f && !isSprintCooldown)
            {
    ...
    private void Crouch()
    {
        ...
        {
            playerCamera.transform.localPosition = new Vector3(playerCamera.transform.localPosition.x, Mathf.Lerp(playerCamera.transform.localPosition.y,-crouchHeight,Time.deltaTime), playerCamera.transform.localPosition.z);
    ...
    private bool CanUnCrouch()
    {
        ...
        // No obstacles found, return true
        return true;
    }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.crouchHeight = EditorGUILayout.Slider(new GUIContent("Crouch Height", "Determines the y scale of the player object when crouched."), fpc.crouchHeight, 1f, 2);
